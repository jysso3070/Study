...
int lua_add_npc_move_event(lua_State *L)
{
	...
	EVENT ev{ npc_id, high_resolution_clock::now() + 1s, EV_MOVE, 0 };
	add_timer(ev);
...
void Create_NPC()
{
	...
	for (int npc_id = NPC_ID_START; npc_id < NPC_ID_START + NUM_NPC; ++npc_id) {
		...
		lua_register(L, "API_add_move_event", lua_add_npc_move_event);
	}
