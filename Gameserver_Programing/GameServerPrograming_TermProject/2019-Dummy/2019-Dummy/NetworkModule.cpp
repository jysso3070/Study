...
#include "..\..\2015184024_Server\2015184024_Server\protocol.h"
...
void ProcessPacket(int ci, unsigned char packet[])
{
	...
	case SC_POS: {
		...
		if (pos_packet->id < NPC_ID_START) {
			int my_id = client_map[pos_packet->id]; // 서버에서 클라이언트에 부여한 id와 더미에서 관리하는 id를 연결해준다
	...
	case SC_PLAYER_DEAD: break;
	case SC_LOGIN_FAIL: break;
	case SC_BATTLE_MESS: break;
	case SC_SEND_ID:
	{
		sc_packet_send_id *id_packet = reinterpret_cast<sc_packet_send_id *>(packet);
		int my_id = client_count++;
		client_map[id_packet->id] = my_id;
		g_clients[my_id].id = id_packet->id;
		break;
	}
	/*case SC_LOGIN_OK:
	{
